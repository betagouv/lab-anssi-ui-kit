<svelte:options
  customElement={{
    ...
    props: {
      ...
      badgesgroup: { attribute: "badgesgroup", type: "Object" },
      buttonsgroup: { attribute: "buttonsgroup", type: "Object" },
      linksgroup: { attribute: "linksgroup", type: "Object" },
    },
  }}
/>
...
<script lang="ts">
  ...
  interface Props {
    ...
    assessBytes: boolean;
    /** Slot permettant d'insérer un groupe de Badges */
    badgesgroup: Snippet;
    /** Slot permettant d'insérer un groupe de Boutons */
    buttonsgroup: Snippet;
    /** Slot permettant d'insérer du contenu dans la zone "detailEnd" */
    contentend: Snippet;
    /** Slot permettant d'insérer un groupe de Liens */
    linksgroup: Snippet;
  }
  ...
  let enlargeClass = $derived.by(() => {
    if (actionMarkup === "button") return "fr-enlarge-button";

    return enlarge && "fr-enlarge-link";
  });
  let detailStartIconClass = $derived.by(() => {
    return hasDetailStartIcon && detailStartIcon && setIconClass(detailStartIcon);
  });
  let detailEndIconClass = $derived.by(() => {
    return hasDetailEndIcon && detailEndIcon && setIconClass(detailEndIcon);
  });
  let sizeClass = $derived(`fr-card--${size}`);
  let horizontalClass = $derived.by(() => horizontal && "fr-card--horizontal");
  let horizontalProportionClass = $derived.by(() => {
    return horizontal && horizontalProportion && `fr-card--horizontal-${horizontalProportion}`;
  });
  let imageRatioClass = $derived(`fr-ratio-${imageRatio}`);
  let variationsClass = $derived(`fr-card--${variations}`);
  let downloadClass = $derived.by(() => download && "fr-card--download");
</script>
