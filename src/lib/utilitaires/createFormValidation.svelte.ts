...
export function createFormValidation() {
   ...
   * @param {boolean} bypassNativeValidation - Permet de "bypasser" la validation native
   */
  function syncFormValue(value: string | undefined, bypassNativeValidation = false) {
    if (!internals || !formControlElement) return;

    internals.setFormValue(value ?? "");

    if (bypassNativeValidation) {
      internals.setValidity({});
    } else {
      updateValidity();
    }
