...
<Story
  ...
  play={async ({ canvasElement, step }) => {
    ...
    await step("Le composant est rendu", async () => {
      const inner = el?.shadowRoot?.querySelector("article");
      await expect(inner).toBeTruthy();
    });
    await step("Le contenu HTML est injecté", async () => {
      const contenu = el?.shadowRoot?.querySelector("article .contenu");
      await expect(contenu).toBeTruthy();
      await expect(contenu?.innerHTML?.length).toBeGreaterThan(0);
    });
    await step("La table des matières est présente", async () => {
      const sommaire = el?.shadowRoot?.querySelector(".sommaire");
      await expect(sommaire).toBeTruthy();
    });
  }}
/>
